...
import { PDF_BREAK_SELECTORS } from '../constants/pdfExport';

export function collectPdfBreakCandidates(root, pageHeightCss) {
...
export function measureEffectiveExportHeight(root) {
...
export function buildPdfSlices(totalHeightCss, pageHeightCss, candidates) {
...
function buildPdfDocument(resolvedOrientation) {
  return new jsPDF(resolvedOrientation === 'landscape' ? 'l' : 'p', 'mm', 'a4');
}
...
export async function exportPortfolioPdf({
  ...
  try {
    ...
    const pdf = buildPdfDocument(resolvedOrientation);
